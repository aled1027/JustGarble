...
/* static int X2A1[8] = { 0x64, 0x78, 0x6E, 0x8C, 0x68, 0x29, 0xDE, 0x60 }; */
/* static int X2S1[8] = { 0x58, 0x2D, 0x9E, 0x0B, 0xDC, 0x04, 0x03, 0x24 }; */
static int S2X1[8] = { 0x8C, 0x79, 0x05, 0xEB, 0x12, 0x04, 0x51, 0x53 };

#define fbits(v, p) ((v & (1 << p)) >> p)

static void
EncoderCircuit(GarbledCircuit *gc, GarblingContext *ctxt, const int *inputs,
               int *outputs, int enc[])
{
	int temp;
	int curWires[8];
	for (int i = 0; i < 8; i++) {
		curWires[i] = fixedZeroWire(gc, ctxt);
	}
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (fbits(enc[i], j)) {
				temp = getNextWire(ctxt);
				XORGate(gc, ctxt, curWires[j], inputs[i], temp);
				curWires[j] = temp;
			}
		}
	}
...
static void
EncoderOneCircuit(GarbledCircuit *gc, GarblingContext *ctxt, const int *inputs,
                  int *outputs, int enc[])
{
	int temp;
	int curWires[8];
	for (int i = 0; i < 8; i++) {
		curWires[i] = fixedOneWire(gc, ctxt);
	}
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (fbits(enc[i], j)) {
...
{
    int temp1[8], temp2[8];
	EncoderCircuit(gc, ctxt, inputs, temp1, A2X1);
...
{
	int outputA = getNextWire(ctxt);
	int outputB = getNextWire(ctxt);
	int outputq0 = getNextWire(ctxt);
	int outputq2 = getNextWire(ctxt);
	int outputq3 = getNextWire(ctxt);
...
INV_GF16(GarbledCircuit *gc, GarblingContext *ctxt, const int *inputs,
         int *outputs)
{
	int AOutput = getNextWire(ctxt);
	int q0Output = getNextWire(ctxt);
	int q1Output = getNextWire(ctxt);
	int q2Output = getNextWire(ctxt);
	int q3Output = getNextWire(ctxt);

	int and01Output = getNextWire(ctxt);
	int and02Output = getNextWire(ctxt);
	int and03Output = getNextWire(ctxt);
	int and12Output = getNextWire(ctxt);
	int and13Output = getNextWire(ctxt);
	int and23Output = getNextWire(ctxt);
	int and012Output = getNextWire(ctxt);
	int and123Output = getNextWire(ctxt);
	int and023Output = getNextWire(ctxt);
	int and013Output = getNextWire(ctxt);
	...
	int tempXORA1 = getNextWire(ctxt);
	XORGate(gc, ctxt, inputs[1], inputs[2], tempXORA1);
	int tempXORA2 = getNextWire(ctxt);
	XORGate(gc, ctxt, inputs[3], and123Output, tempXORA2);
	XORGate(gc, ctxt, tempXORA1, tempXORA2, AOutput);

	int tempXORq01 = getNextWire(ctxt);
	int tempXORq02 = getNextWire(ctxt);
	int tempXORq03 = getNextWire(ctxt);
	...
	int tempXORq11 = getNextWire(ctxt);
	int tempXORq12 = getNextWire(ctxt);
	int tempXORq13 = getNextWire(ctxt);
	int tempXORq14 = getNextWire(ctxt);
	/* int tempXORq15 = */ getNextWire(ctxt);
	...
	int tempXORq21 = getNextWire(ctxt);
	int tempXORq22 = getNextWire(ctxt);
	int tempXORq23 = getNextWire(ctxt);
	int tempXORq24 = getNextWire(ctxt);
	/* int tempXORq25 = */ getNextWire(ctxt);
	...
	int tempXORq31 = getNextWire(ctxt);
	int tempXORq32 = getNextWire(ctxt);
	/* int tempXORq33 = */ getNextWire(ctxt);
	/* int tempXORq34 = */ getNextWire(ctxt);
	/* int tempXORq35 = */ getNextWire(ctxt);
...
{
	int AOutput = getNextWire(ctxt);
	int BOutput = getNextWire(ctxt);
	int COutput = getNextWire(ctxt);
	int DOutput = getNextWire(ctxt);

	int q0Output = getNextWire(ctxt);
	int q1Output = getNextWire(ctxt);
	int q2Output = getNextWire(ctxt);
	int q3Output = getNextWire(ctxt);
	int q4Output = getNextWire(ctxt);
	int q5Output = getNextWire(ctxt);
	int q6Output = getNextWire(ctxt);
	int q7Output = getNextWire(ctxt);

	int a0barOutput = getNextWire(ctxt);
	int a1barOutput = getNextWire(ctxt);
	int a5barOutput = getNextWire(ctxt);
	int a6barOutput = getNextWire(ctxt);
